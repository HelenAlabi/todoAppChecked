...
let displayFunc = ()=>{
    ...
    if (storage[i].checked == true){
        ...
        html  += 
        `<li id="list">
        <input type="checkbox" id='check${i}' onchange="strike(${i})"> ${storage[i].name} <button onclick="editTask()" id='edit'>Edit</button> <button onclick="deleteTask()" id 'del'>Delete</button>
        </li>`
    }
}
